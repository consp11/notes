...
sudo nice -n 19 clamscan -iozr --follow-dir-symlinks=2 --follow-file-symlinks=2 --max-filesize=12M --max-scansize=12M --max-recursion=128 --max-dir-recursion=128 --alert-encrypted=no --alert-broken=yes --alert-exceeds-max=no --max-files=1000000 --pcre-match-limit=200000 --pcre-recmatch-limit=10000 --pcre-max-filesize=12M --bytecode-timeout=600000 --scan-archive=no --scan-xmldocs=no --scan-hwp3=no --scan-html=no --scan-swf=no --scan-pdf=no --scan-pe=no --phishing-scan-urls=no --scan-mail=no --phishing-sigs=no --scan-elf=yes /usr/sbin /usr/bin /usr/**/bin /usr/**/sbin /opt /boot &>> /inRam/clamav.log &
